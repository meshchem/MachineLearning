...
def addition(min_val=0, max_val=100):
...
def multiplication(min_val=0, max_val=10):
    a = random.randint(min_val, max_val)
    b = random.randint(min_val, max_val)
    mul = OPS['*']
    # avoid division by zero
...
# def division(min_val=1, max_val=10):
#     b = random.randint(min_val, max_val)

#     # ensure multiplier >= 1 and keeps a <= max_val
#     max_mult = max_val // b
#     if max_mult < 1:
#         max_mult = 1

    # multiplier = random.randint(1, max_mult)
    # a = b * multiplier

    # expr = f"{a}/{b}"
    # result = str(a // b)   # INTEGER result as STRING

    # return expr, result
def division(min_divisor=1, max_divisor=10, max_quotient=10):
    q = random.randint(1, max_quotient)
...
def generate_dataset(n_simple=10000, n_paren=2000, path="addition_dataset.csv"):
    rows = []
